...
if(statsonly)
{
  ...
  fprintf(sFiles[i],"Marker\tChromosome\tPosition\tAlleleA\tAlleleB\tAA_all\tAB_all\tBB_all\tGEN_all_err\tMAC_all\tMAF_all\tInfo_all\n");
  }
}
...
else
{
  ...
  Stat="R2"; 
  for(i = 0; i < phenorows; i++) {
  fprintf(sFiles[i],"Marker\tChromosome\tPosition\tAlleleA\tAlleleB\tPheno\tPhenoCount\tAA_all\tAB_all\tBB_all\tGEN_all_err\tMAC_all\tAA_pheno\tAB_pheno\tBB_pheno\tGEN_pheno_err\tMAC_pheno\tSlope\tR2");//V04
  }
  }
  ...
  Stat="T";
  for(i = 0; i < phenorows; i++) {
  fprintf(sFiles[i],"Marker\tChromosome\tPosition\tAlleleA\tAlleleB\tPheno\tPhenoCount\tAA_all\tAB_all\tBB_all\tGEN_all_err\tMAC_all\tAA_pheno\tAB_pheno\tBB_pheno\tGEN_pheno_err\tMAC_pheno\tSlope\tSEslope\tR2\tT");//V04
  }
  }
  ...
  Stat="P";
  for(i = 0; i < phenorows; i++) {
  fprintf(sFiles[i],"Marker\tChromosome\tPosition\tAlleleA\tAlleleB\tPheno\tPhenoCount\tAA_all\tAB_all\tBB_all\tGEN_all_err\tMAC_all\tAA_pheno\tAB_pheno\tBB_pheno\tGEN_pheno_err\tMAC_pheno\tSlope\tSEslope\tR2\tT\tP");//V04
  }
  }

  else Stat="stat";

  if(info) for(i = 0; i < phenorows; i++) {fprintf(sFiles[i],"\tInfo_all");}
  if(info2) for(i = 0; i < phenorows; i++) {fprintf(sFiles[i],"\tInfo_pheno");}
  for(i = 0; i < phenorows; i++) {fprintf(sFiles[i],"\n");}
	   ...
	   if(abs_slope>=slope_lim && MAC2>=mac_lim && goodphenos>1)//0.4
           {
             ...
             {    
              fprintf(sFiles[counter],"%s\t%s\t%s\t%s\t%s\t%s\t%i\t%f\t%f\t%f\t%i\t%f\t%f\t%f\t%f\t%i\t%f\t",rsnumber,chrom,rsposition,alleleA,alleleB,phenotypename,goodphenos,AA,AB,BB,XX,MAC,AV1,AV2,AV3,AVXX,MAC2);//V04
	      
	      if((MAC2<=0.0 || slope==0.0) && info) fprintf(sFiles[counter],"NA\tNA\t1\n");
	      else if ((MAC2<=0.0 || slope==0.0) && !info) fprintf(sFiles[counter],"NA\tNA\n");
	      else
	      {	
	      fprintf(sFiles[counter],"%f",slope);
	      ...
	      if(info2 && INFOSCORE2>=0) fprintf(sFiles[counter],"\t%f",INFOSCORE2); 
		   ...
		   if(SE<=se_lim)//0.4
	        ...
	        else if ((MAC2<=0.0 || slope==0.0) && !info) fprintf(sFiles[counter],"NA\tNA\tNA\tNA\n");
		 
	         else
	         {	
		 fprintf(sFiles[counter],"%f\t%f",slope,SE);
                 if(R2>0) fprintf(sFiles[counter],"\t%.3e",R2); else fprintf(sFiles[counter],"\tNA");
                 if(T>0) fprintf(sFiles[counter],"\t%f",T); else fprintf(sFiles[counter],"\tNA");
		 ...
		 if(info && INFOSCORE>=0) fprintf(sFiles[counter],"\t%f",INFOSCORE); 
             ...
             if(statistic==2)   
             {
			  ...
			  if((MAC2<=0.0 || slope==0.0) && info) fprintf(sFiles[counter],"NA\tNA\tNA\tNA\tNA\t1\n");
	                  else if ((MAC2<=0.0 || slope==0.0) && !info) fprintf(sFiles[counter],"NA\tNA\tNA\tNA\tNA\n");
